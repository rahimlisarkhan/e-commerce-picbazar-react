...
export const getProducts = (n = 1) => (dispatch) => {
  ...
  dispatch({ type: type.IS_LOADING_CARD, payload: true });
  dispatch({ type: type.GET_PRODUCTS, payload: [] });

  baseURL
    .get(`/api/products/?page=${n}&page_size=10&ordering=-created_at`)
    .then((res) => {
      dispatch({ type: type.GET_PRODUCTS, payload: res.data.products });
      dispatch({ type: type.GET_PRODUCTS_LENGTH, payload: res.data.count });
      dispatch({ type: type.IS_LOADING_CARD, payload: false });
    })
    .catch((err) => toast.error(err))
    .finally(() => dispatch({ type: type.IS_LOADING_CARD, payload: false }));
};
...
export const getAllProducts = (n = 1) => (dispatch) => {
  ...
  baseURL
    .get(`/api/products/?page=${n}&page_size=5&ordering=-created_at`)
    .then((res) => {
...
export const getProductCategories = (id) => (dispatch) => {
  ...
  baseURL
    ...
    .then((res) => {
      dispatch({
        type: type.GET_PRODUCTS_CATEGORIES,
...
export const basketProductRemove = (productID) => (dispatch) => {
  ...
  baseURL.delete(`/api/basket/${productID}/`).then((res) => {
    console.log(res);
...
export const createAddProduct = (data) => (dispatch) => {
  ...
  baseURL
    ...
    .catch((err) => toast.error(err))
    .finally(() => dispatch({ type: type.GET_LOGIN_PAGE, payload: false }));
};
